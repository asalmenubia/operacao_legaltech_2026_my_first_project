...
CREATE FUNCTION staging.normalized_token(value text) RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE
RETURN nullif(regexp_replace(lower(trim(value)), '[^a-z0-9]+', '_', 'g'), '');

CREATE FUNCTION staging.source_date(value text) RETURNS date
LANGUAGE sql IMMUTABLE PARALLEL SAFE
RETURN CASE
...
CREATE FUNCTION staging.source_boolean(value text) RETURNS boolean
LANGUAGE sql IMMUTABLE PARALLEL SAFE
...
CREATE FUNCTION staging.source_channel(value text) RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE
RETURN (regexp_match(lower(trim(value)), '^(whatsapp|phone|email)'))[1];

CREATE PROCEDURE staging.promote_batch(p_load_batch_id bigint)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    WHEN 'matters' THEN
        ...
        SELECT matter_id::bigint, client_id::bigint, nullif(consultation_id, '')::bigint,
               ...
               replace(staging.normalized_token(outcome), 'client_withdraw', 'client_withdrawal')
